...
public class Main212BookingAgent {
    public static void main(String[] args) {
        int rows = 10;
        int totalSeats = 100;
        Theatre212 rodgersNYC = new Theatre212("Richards rodgers", rows, totalSeats);
    ...
    private static void bookSeat(Theatre212 theatre, char row, int seatNo) {
    ...
    private static void bookSeats(Theatre212 theatre, int tickets, char minRow, int minSeat, int maxSeat) {
        bookSeats(theatre, tickets, minRow, minRow, minSeat, maxSeat);
    }

    private static void bookSeats(Theatre212 theatre, int tickets, char minRow, char maxRow, int minSeat, int maxSeat) {
        var seats = theatre.reserveSeats(tickets, minRow, maxRow, minSeat, maxSeat);
        if (seats != null) {
            System.out.println("Congratulations! Your reserved seats is " + seats);
            theatre.printSeatMap();
        } else {
