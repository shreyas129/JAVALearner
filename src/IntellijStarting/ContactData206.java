...
public class ContactData206 {
    ...
    public static List<Contact206> getData(String type) {
        List<Contact206> dataList = new ArrayList<>();
        Scanner scanner = new Scanner(type.equals("phone") ? phoneData : emailData);
        while (scanner.hasNext()) {
            String[] data = scanner.nextLine().split(",");
            Arrays.asList(data).replaceAll(String::trim);
            if (type.equalsIgnoreCase("phone")) {
                dataList.add(new Contact206(data[0], Long.parseLong(data[1])));
            } else if (type.equalsIgnoreCase("email")) {
                dataList.add(new Contact206(data[0], data[1]));
            }
        }
        return dataList;
    }
